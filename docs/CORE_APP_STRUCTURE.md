...
**Related docs:** [ARCHITECTURE_AWS_GCP_GENERAL.md](learned/cloud_shared/ARCHITECTURE_AWS_GCP_GENERAL.md), [ANALYTICS_AND_DATA.md](learned/cloud_shared/ANALYTICS_AND_DATA.md), [CODE_STRUCTURE_WITH_AWS.md](CODE_STRUCTURE_WITH_AWS.md), [war_stories/](../war_stories/README.md).
...
Two main data flows feed Aurora:

1. **Embeddings flow (left):** CSV → ETL → `fru_sales_embeddings` → API `/query` (semantic search)
2. **Analytics flow (right):** S3 Delta → Spark → `batch_analytics` → API `/analytics`

The embeddings flow has **two paths**: AWS (RDS Data API) and local (psycopg2), depending on whether the DB is reachable via TCP.

### 3.2 Diagram (with data types on edges)

```mermaid
%%{init: {'themeVariables': {'fontSize':'8px','fontFamily':'arial'}}}%%
flowchart TB
    subgraph sources["Data Sources"]
        csv["CSV<br/><small>fridge_sales_with_rating.csv</small>"]
        delta["S3 Delta<br/><small>s3a://.../delta/fru_sales</small>"]
    end

    subgraph etl_aws["ETL — AWS (Deploy Phase 6)"]
        setup_db["setup_database.py"]
        rds_api["load_openai_embeddings_to_pgvector_rds_api.py"]
    end

    subgraph etl_local["ETL — Local / TCP-accessible DB"]
        psycopg["load_openai_embeddings_to_pgvector.py"]
    end
    ...
    delta -->|"Parquet files"| run_analytics
    run_analytics -->|"sales_by_brand, store_perf, etc."| save_db
    save_db -->|"JSONB aggregates"| batch_analytics

    fru_emb -->|"vector similarity"| query_ep
    batch_analytics -->|"latest row"| analytics_ep

    style csv fill:#e3f2fd
    style delta fill:#e3f2fd
    style rds_api fill:#fff3e0
    style psycopg fill:#f3e5f5
...
| 9 | fru_sales_embeddings → /query | Vector similarity results | API runs semantic search over embeddings |
| 10 | batch_analytics → /analytics | Latest row (JSONB columns) | API `SELECT * FROM batch_analytics ORDER BY created_at DESC LIMIT 1` |

### 3.4 Local vs AWS Branch (B.1.a)

The **right branch** (dashed) is for **local and TCP-accessible databases**:

- **`load_openai_embeddings_to_pgvector.py`** uses psycopg2 with `PGHOST`, `PGPORT`, `PGUSER`, `PGPASSWORD`, `PGDATABASE`
- Use when: local PostgreSQL, or Aurora reachable via port-forward / bastion / same VPC from your machine
- **`load_openai_embeddings_to_pgvector_rds_api.py`** uses RDS Data API (HTTPS); no TCP needed
- Use when: AWS deploy from laptop/CI; Aurora in private subnet

Both write to the same `fru_sales_embeddings` table; only the connection method differs.

### 3.5 Table Summary

| Table | Written By | Read By | Purpose |
|-------|------------|---------|---------|
| **fru_sales_embeddings** | RDS API ETL (AWS) or psycopg2 ETL (local) | API `/query` (semantic search) | Embeddings for fridge sales feedback |
...
| Topic | Doc |
|-------|-----|
| Kube / Nonkube architecture (AWS & GCP) | [ARCHITECTURE_AWS_GCP_GENERAL.md](learned/cloud_shared/ARCHITECTURE_AWS_GCP_GENERAL.md) |
| Shared data (batch_analytics, Delta) | [ANALYTICS_AND_DATA.md](learned/cloud_shared/ANALYTICS_AND_DATA.md) |
