...
- **Kube** uses `kube_apply.py` to apply manifests from `infra_modules/cloud_shared/k8s/` (bootstrap-job.yaml, spark-cronjob.yaml). These run in EKS.
...
|-------|-----------|---------------|
| `live_deploy_aws/scope_shared/durable` | `fru/{env}/aws-shared-durable.tfstate` | VPC, subnets |
| `live_deploy_aws/scope_shared/nondurable` | `fru/{env}/aws-shared-nondurable.tfstate` | S3 (delta, artifacts), ECR (app, spark) |
| `live_deploy_aws/nonkube` | `fru/{env}/aws-nonkube.tfstate` | ECS, ALB, frontend (CloudFront, S3, OAC), EventBridge, IAM, CloudWatch |
| `live_deploy_aws/kube` | `fru/{env}/aws-kube.tfstate` | EKS, frontend (CloudFront, S3, OAC) |
...
- [x] Update: `infra_modules/cloud_shared/k8s/bootstrap-job.yaml`, `spark-cronjob.yaml`
