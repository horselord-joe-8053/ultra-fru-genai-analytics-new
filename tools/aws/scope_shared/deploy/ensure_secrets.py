...
def init_stack(env, region=None):
    ...
    cfg = backend_config("infra_terraform/live_deploy/aws/scope_shared/durable", env, region)
    args = ["init", "-lock=false", "-upgrade", "-reconfigure"]
    for c in cfg:
        args += ["-backend-config", c]
    exe = os.getenv("FRU_TF_BIN", "tofu")
    cmd = [exe] + args
    run_with_retry(cmd, cwd="infra_terraform/live_deploy/aws/scope_shared/durable", env=get_terra_env(region), description="tofu init for secrets")
...
def outputs(env, region=None):
    ...
    out = subprocess.check_output([os.getenv("FRU_TF_BIN","tofu"),"output","-json"], cwd="infra_terraform/live_deploy/aws/scope_shared/durable", text=True, timeout=30, env=get_terra_env(region))
