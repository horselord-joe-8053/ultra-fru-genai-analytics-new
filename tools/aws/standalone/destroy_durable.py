...
def init_stack(env):
    cfg = backend_config("infra_terraform/live_deploy/aws/scope_shared/durable", env, region=None, cloud="aws")
    args = ["init","-upgrade"]
    for c in cfg:
...
def main():
    ...
    ] + base, cwd="infra_terraform/live_deploy/aws/scope_shared/durable", check=True)
