...
function shouldSkipMessage(msg) {
  ...
  if (isDeepResearchWidgetMessage(meta)) return true;
  return false;
}

function isDeepResearchWidgetMessage(meta) {
  const resourceUri =
    meta.invoked_resource &&
    typeof meta.invoked_resource === "object" &&
    typeof meta.invoked_resource.resource_uri === "string"
      ? meta.invoked_resource.resource_uri
      : "";
  if (resourceUri.includes("Deep Research App")) return true;
  if (meta.venus_message === true) return true;
  if (typeof meta.venus_message_type === "string" && meta.venus_message_type) {
    return true;
  }
  const toolMeta =
    meta.chatgpt_sdk &&
    typeof meta.chatgpt_sdk === "object" &&
    meta.chatgpt_sdk.tool_response_metadata &&
    typeof meta.chatgpt_sdk.tool_response_metadata === "object"
      ? meta.chatgpt_sdk.tool_response_metadata
      : null;
  if (!toolMeta) return false;
  if (toolMeta.venus_message === true) return true;
  if (
    toolMeta.venus_widget_state &&
    typeof toolMeta.venus_widget_state === "object"
  ) {
    return true;
  }
  return false;
